...
void load_tool(int idx) {
    ...
    if(random_toolchanger) {
        ...
        CANON_TOOL_TABLE  tzero   = tooldata_get(0);
        CANON_TOOL_TABLE  tpocket = tooldata_get(idx);
...
int read_tool_inputs(void)
{
    ...
    if (*iocontrol_data->tool_change && *iocontrol_data->tool_changed) {
        if(!random_toolchanger && emcioStatus.tool.pocketPrepped == 0) {
            ...
            CANON_TOOL_TABLE tdata = tooldata_get(emcioStatus.tool.pocketPrepped);
...
int main(int argc, char *argv[])
{
    ...
    if (random_toolchanger) {
        CANON_TOOL_TABLE tdata = tooldata_get(0);
        emcioStatus.tool.toolInSpindle = tdata.toolno;
    } else {
        emcioStatus.tool.toolInSpindle = 0;
    }
        ...
        case EMC_TOOL_SET_OFFSET_TYPE: 
            {
                ...
                CANON_TOOL_TABLE tdata = tooldata_get(idx);
                tdata.toolno = toolno;
                tdata.offset = offs;
                tdata.diameter = d;
                tdata.frontangle = f;
